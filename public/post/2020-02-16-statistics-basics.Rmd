...
Statistics is inherent in laboratory work since our primary product is data, yet so little emphasis is placed on it in medical technology schools. In this post we will cover basic statistical concepts that are widely used in the laboratory. We still study data types and their associated probability distributions. In the process we learn about measures of central tendency (i.e. mean, mode, median) and measures of dispersion (i.e. variance, standard deviation, coefficient of variation).

# Data Types

Data can generally can be classified as:
  
  1. **Discrete** - there are no between values.
      a. Ordered (values are comparable)
          - Ordinal (e.g. size: S, M, L, XL, ...)
          - Quantitative (e.g. counts: 1, 2, 3, ...)
      b. Unordered (values are not comparable)
          - Nominal (e.g. shape: $\square$, $\diamond$, $\circ$, ...)
          - Categories (e.g. WBC type: neutrophil, lymphocyte, monocyte, ...)
  2. **Continuous** - there are between values.
       a. Ordered (values are comparable)
          - Fields (e.g. sodium level, temperature)
       b. Unordered (values are not comparable)
          - Cyclic values (e.g. directions)
...
```{r echo=FALSE, fig.width=2, fig.height=2, fig.align='center', error=FALSE, warning=FALSE}
set.seed(1)
...
The mode the most frequent value in a series of points. Mode is most often used for categorical or discrete data. 
...
One of the issues of mode is that it is not always unique. We could for example have the same count for neutrophils and lymphocytes when performing a differential. 
...
This is why mode is seldom used for continuous variables. For example, consider measuring the glucose level of 30 people. How likely is it for two or more people to have **exactly** the same glucose level (e.g. 94.5 mmol/L)? It is very unlikely.
